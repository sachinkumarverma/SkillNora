...
const listAdmin = async (req, res) => {
  try {
    ...
    if (error) {
      return res
        .status(401)
        .json({ error: "Unauthorized: Auth Error", details: error.message });
    }
    if (!userData?.user) {
      return res
        .status(401)
        .json({ error: "Unauthorized: User not found from token" });
    }
...
const bulkPublish = async (req, res) => {
  try {
    ...
    await coursesService.bulkPublish(
      userData.user.id,
      req.body.ids,
      req.body.status,
    );
...
const updateLectures = async (req, res) => {
  try {
    const token = req.headers.authorization?.split(" ")[1];
    const { data: userData } = await supabaseServer.auth.getUser(token);
    await coursesService.updateLectures(
      userData.user.id,
      req.params.id,
      req.body.lectures,
    );
...
const complete = async (req, res) => {
  try {
    ...
    const { courseId, slug, lectureId, totalLectures, quizScore } = req.body;
    if (!courseId || !slug || !lectureId || !totalLectures)
      return res.status(400).json({
        error: "Missing parameters",
      });
    const result = await coursesService.completeLecture(
      userData.user.id,
      courseId,
      slug,
      lectureId,
      ...
      if (enrollments && enrollments.length > 0) {
        ...
        for (const enr of enrollments) {
          ...
          const percentRemaining = Math.max(
            0,
            Math.min(1, remainingMs / msInYear),
          );
          const refundAmount =
            Math.round(coursePrice * percentRemaining * 100) / 100;
...
const addReview = async (req, res) => {
  try {
    ...
    const result = await coursesService.addReview(
      userData.user.id,
      courseId,
      rating,
      review_text,
...
const updateReview = async (req, res) => {
  try {
    ...
    const result = await coursesService.updateReview(
      userData.user.id,
      reviewId,
      rating,
      review_text,
