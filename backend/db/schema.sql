...
create policy "Anyone can read comments" on lecture_comments for select using (true);
create policy "Authenticated users can insert comments" on lecture_comments for insert with check (auth.uid() = user_id);
create policy "Users can delete own comments" on lecture_comments for delete using (auth.uid() = user_id);
