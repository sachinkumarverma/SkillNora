...
export default function InstructorCourseBuilder() {
    ...
    useEffect(() => {
        const fetchUserAndInstructors = async () => {
            ...
            if (editCourseId) {
                const { data: c } = await supabase.from('courses').select('*').eq('id', editCourseId).single()
                if (c) {
                    setCourseData({
                        title: c.title || '',
    ...
    const addModule = () => {
        setModules([...modules, { id: Date.now(), title: 'New Module', videoMode: 'upload', videoUrl: '', thumbnailMode: 'upload', thumbnailUrl: '' }])
    }
    ...
    // Helper to get selected instructor label
    const getSelectedInstructorLabel = () => {
        ...
        return inst ? (inst.full_name || inst.email) : 'Select an Instructor'
    }
        ...
        <form onSubmit={handlePublish} className="max-w-5xl mx-auto p-6 lg:p-8 pb-32">
                ...
                {showSuccessModal && (
                    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-sm p-4">
                        ...
                        <motion.div 
                            ...
                            className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-10 max-w-md w-full shadow-2xl text-center relative overflow-hidden"
                        >
                            ...
                            >
                                <button type="button" onClick={() => router.push('/admin/courses')} className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-6 rounded-2xl shadow-lg shadow-blue-500/30 transition-transform active:scale-95">
            ...
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
                ...
                <div className="flex gap-3">
                    <button type="button" className="px-5 py-2.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 font-bold hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors shadow-sm text-sm">
                        Save as Draft
                    </button>
                    <button type="submit" disabled={isSaving} className={`px-5 py-2.5 rounded-full text-white font-bold transition-colors shadow-sm text-sm flex items-center gap-2 ${isSaving ? 'bg-blue-400 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700'}`}>
                ...
                <div className="lg:col-span-2 space-y-8">
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <div className="flex bg-slate-100 dark:bg-slate-800 p-1 rounded-lg">
                                <button onClick={() => setThumbnailMode('upload')} className={`px-3 py-1 rounded-md text-xs font-bold transition-colors ${thumbnailMode === 'upload' ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Upload</button>
                                <button onClick={() => setThumbnailMode('unsplash')} className={`px-3 py-1 rounded-md text-xs font-bold transition-colors ${thumbnailMode === 'unsplash' ? 'bg-white dark:bg-slate-600 text-slate-900 dark:text-white shadow-sm' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Unsplash URL</button>
                            </div>
                        </div>
                    ...
                    >
                        <div className="flex items-center justify-between mb-6">
                            ...
                            <button onClick={addModule} className="px-4 py-2 bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400 rounded-full font-bold text-sm hover:bg-blue-100 dark:hover:bg-blue-900/40 transition-colors flex items-center gap-2 shrink-0">
                        ...
                        <div className="space-y-6">
                            {modules.map((mod, index) => (
                                <div key={mod.id} className="border border-slate-200 dark:border-slate-700 rounded-2xl overflow-hidden bg-slate-50/50 dark:bg-slate-800/20 shadow-sm">
                                    <div className="flex items-center justify-between p-4 bg-slate-100/80 dark:bg-slate-800/80 border-b border-slate-200 dark:border-slate-700">
                                        ...
                                        {modules.length > 1 && (
                                            <button onClick={() => removeModule(mod.id)} className="text-slate-400 hover:text-red-500 transition-colors p-1">
                                        ...
                                        <div>
                                            <div className="flex items-center justify-between mb-2">
                                                ...
                                                <div className="flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-1 rounded-lg">
                                                    <button onClick={() => updateModule(mod.id, 'videoMode', 'upload')} className={`px-2 py-1 rounded text-xs font-bold transition-colors ${mod.videoMode === 'upload' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Upload</button>
                                                    <button onClick={() => updateModule(mod.id, 'videoMode', 'link')} className={`px-2 py-1 rounded text-xs font-bold transition-colors ${mod.videoMode === 'link' ? 'bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>YouTube Link</button>
                                                </div>
                                            </div>
                                        ...
                                        {mod.videoMode === 'upload' ? (
                                            <div>
                                                <div className="flex items-center justify-between mb-2">
                                                    ...
                                                    <div className="flex bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 p-1 rounded-lg">
                                                        <button onClick={() => updateModule(mod.id, 'thumbnailMode', 'upload')} className={`px-2 py-1 rounded text-xs font-bold transition-colors ${mod.thumbnailMode === 'upload' ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Upload</button>
                                                        <button onClick={() => updateModule(mod.id, 'thumbnailMode', 'unsplash')} className={`px-2 py-1 rounded text-xs font-bold transition-colors ${mod.thumbnailMode === 'unsplash' ? 'bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400' : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300'}`}>Unsplash Link</button>
                                                    </div>
                                                </div>
