...
export default function Hero() {
    return (
        <section className='overflow-hidden rounded-xl glass'>
                    ...
                    <div className='surface rounded-[1.75rem] p-5 shadow-2xl shadow-slate-950/10'>
                        ...
                        <div className='mt-5 grid grid-cols-2 gap-3 text-sm'>
                            {['Roadmaps', 'Quizzes', 'Certificates', 'Mentorship'].map((item) => (
                                <div key={item} className='rounded-lg bg-slate-100 px-4 py-3 font-medium text-slate-700 dark:bg-slate-800 dark:text-slate-200'>{item}</div>
                            ))}
                        </div>
                        <div className='mt-5 rounded-xl bg-gradient-to-br from-blue-600 via-cyan-500 to-violet-600 p-5 text-white'>
                    ...
                    <div className='surface rounded-[1.5rem] p-5'>
                        ...
                        <div className='mt-3 grid gap-3 sm:grid-cols-2'>
                            {['AI for product teams', 'Leadership with systems'].map((course) => (
                                <div key={course} className='rounded-lg border border-slate-200/60 bg-white p-4 dark:border-slate-700 dark:bg-slate-900'>
