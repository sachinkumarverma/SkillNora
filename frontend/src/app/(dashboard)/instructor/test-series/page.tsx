...
export default function InstructorTestSeries() {
    ...
    const fetchSeries = () => {
        setLoading(true)
        apiClient.get('/api/instructor/test-series')
            .then(res => {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        setIsSaving(true)
        try {
            await apiClient.post('/api/instructor/test-series', formData)
            setIsCreating(false)
    ...
    const executeDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            await apiClient.delete(`/api/instructor/test-series/${seriesToDelete}`)
            fetchSeries()
                    ...
                    <div key={s.id} className="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col">
                        ...
                        <div className="p-6 flex flex-col flex-1 min-w-0">
                            ...
                            <div className="mt-auto grid grid-cols-2 gap-3">
                                ...
                                <Link href={`/instructor/test-series/${s.id}`} className="flex items-center justify-center gap-2 py-2.5 bg-blue-600 text-white hover:bg-blue-700 rounded-xl font-bold transition-colors">
