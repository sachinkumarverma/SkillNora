...
export default function CreateTestPage() {
    ...
    const [formData, setFormData] = useState({ title: '', duration_minutes: '30', total_marks: '30' })
    ...
    const generateMCQs = async () => {
        ...
        try {
            const res = await apiClient.post('/api/ai/chat', {
                ...
                options: {
                    systemPromptOverride: 'You are an instructional designer. Return a JSON object with a "questions" key containing exactly 30 multiple-choice questions. You MUST generate all 30 questions. Include a "question_number" from 1 to 30 in each object to keep track. Each object must have: "question_number" (integer), "question" (string), "options" (array of exactly 4 strings), and "correctIndex" (integer 0-3). Return ONLY valid JSON.',
                    maxTokens: 8000,
                    useJsonFormat: true,
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        setIsSaving(true)
        try {
            await apiClient.post(`/api/instructor/test-series/${id}/tests`, {
                ...formData,
            ...
            router.push(`/instructor/test-series/${id}`)
        } catch (error) {
    ...
    return (
        <div className="w-full mx-auto p-4 sm:p-6 lg:p-8">
            <div className="mb-6">
                <Link href={`/instructor/test-series/${id}`} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors w-fit">
            ...
            <form onSubmit={handleCreate} className="space-y-6">
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                        <Link href={isSaving ? "#" : `/instructor/test-series/${id}`} className={`px-6 py-2.5 rounded-xl font-bold text-slate-600 bg-slate-100 dark:bg-slate-800 dark:text-slate-300 transition-colors ${isSaving ? 'opacity-50 cursor-not-allowed pointer-events-none' : 'hover:bg-slate-200 dark:hover:bg-slate-700'}`}>Cancel</Link>
                        <button type="submit" disabled={isSaving} className="flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors shadow-md shadow-blue-500/20">
                            {isSaving ? (
                                <>
                            ...
                            <div key={qIdx} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl relative group">
                                ...
                                <div className="p-4 pt-6">
                                    <div className="flex items-center gap-3 mb-4">
                                        <div className="flex items-center justify-center w-9 h-9 rounded-lg bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 font-bold text-sm shrink-0 border border-slate-200 dark:border-slate-700">
                                            Q{qIdx + 1}
                                        </div>
                                        <input type="text" value={mcq.question} onChange={e => {
                                            ...
                                            setQuestions(newQuestions);
                                        }} placeholder="Question text..." className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 dark:text-slate-200 focus:border-blue-500 outline-none" />
                                    </div>
