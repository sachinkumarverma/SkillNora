...
export default function AdminCertificatesPage() {
    ...
    const fetchCertificates = async () => {
        setLoading(true)
        try {
            const res = await apiClient.get('/api/admin/certificates')
    ...
    if (loading && certificates.length === 0) return <Loader type="management-table" />
        ...
        <div className="w-full mx-auto p-6 lg:p-8 space-y-8 pb-20">
            <header className="flex flex-col md:flex-row md:items-end justify-between gap-4">
                <div>
                    ...
                    >
                        Track and verify issued certificates across the platform.
                    </motion.p>
                </div>
            </header>
