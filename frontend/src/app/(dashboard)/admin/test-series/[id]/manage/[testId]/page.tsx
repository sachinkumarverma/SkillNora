...
export default function InstructorTestQuestions() {
    ...
    const handleCreate = async (e: React.FormEvent) => {
        ...
        try {
            if (editingId) {
                await apiClient.put(`/api/admin/test-series/tests/${testId}/questions/${editingId}`, data)
                toast.success("Question updated successfully!")
            } else {
                await apiClient.post(`/api/admin/test-series/tests/${testId}/questions`, data)
    ...
    const executeDeleteQuestion = async () => {
        ...
        try {
            await apiClient.delete(`/api/admin/test-series/tests/${testId}/questions/${questionToDelete}`)
            fetchTest()
    ...
    return (
        <div className="w-full mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
            <div>
                <button onClick={() => router.push(`/admin/test-series/${id}`)} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors mb-4">
