...
export default function InstructorTestSeriesDetails() {
    ...
    const executeDelete = async () => {
        ...
        setIsDeleting(true)
        try {
            await apiClient.delete(`/api/admin/test-series/tests/${testToDelete}`)
            fetchSeries()
    ...
    const togglePublish = async () => {
        try {
            await apiClient.put(`/api/admin/test-series/${id}`, { ...series, is_published: !series.is_published })
            fetchSeries()
        } catch (error) {
    ...
    return (
        <div className="w-full mx-auto p-4 sm:p-6 lg:p-8">
            <div className="mb-6">
                <button onClick={() => router.push('/admin/test-series')} className="flex items-center gap-2 text-sm font-bold text-slate-500 hover:text-blue-600 transition-colors">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end mb-6 gap-4">
                ...
                <div className="flex flex-col sm:flex-row items-center gap-3">
                    ...
                    <Link href={`/admin/test-series/${id}/new-test`} className="w-full sm:w-auto flex items-center justify-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 px-5 rounded-xl transition-colors shadow-md shadow-blue-500/20">
                ...
                {series.tests?.map((t: any) => (
                    <div key={t.id} className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl p-5 flex flex-col sm:flex-row gap-4 items-center justify-between hover:border-blue-300 transition-colors shadow-sm">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Link href={`/admin/test-series/${id}/manage/${t.id}`} className="p-2 text-slate-400 hover:text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors" title="Edit Questions">
