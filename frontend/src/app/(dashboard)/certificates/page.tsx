        ...
        <div className="max-w-[1400px] mx-auto px-6 py-12">
            ...
            {certs.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {certs.map((cert, i) => (
                        <div key={i} className="relative bg-white dark:bg-slate-900 rounded-lg p-6 border border-slate-200 dark:border-slate-800 shadow-sm flex flex-col items-center text-center transform transition hover:-translate-y-1 hover:shadow-md overflow-hidden z-10 group">
                            {/* Watermark */}
                            ...
                            <button onClick={() => router.push(`/certificates/${cert.courseSlug || 'cert'}/${cert.id}`)} className="w-full mt-auto bg-slate-100 dark:bg-slate-800 text-blue-600 dark:text-blue-400 font-bold py-2.5 rounded-xl hover:bg-slate-200 dark:hover:bg-slate-700 transition-colors">
                                View Certificate
                            </button>
                        </div>
                    ))}
